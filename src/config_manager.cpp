...
void ConfigManager::SetModel(const std::string& model_name) {
    AppConfig config = Load();
    config.model = model_name;
    Save(config);
    LOG_INFO("已将默认模型设置为: %s", model_name.c_str());
}

void ConfigManager::SetUrl(const std::string& url) {
    AppConfig config = Load();
    config.ollama_url = url;
    Save(config);
    LOG_INFO("已将默认 Ollama URL 设置为: %s", url.c_str());
}